...
/* bus_info of ports */
static const char* bus_info[RTE_MAX_ETHPORTS];

/* output_port */
static unsigned int output_port[RTE_MAX_ETHPORTS];

/* ip addresses of ports */
static uint8_t port_ip_addr[RTE_MAX_ETHPORTS][4];

/* port mtu */
static uint16_t port_mtu[RTE_MAX_ETHPORTS];
...
print_stats(void)
{
	...
	sec_counter++;
		...
		printf(" BUS %s", bus_info[portid]);

		printf(" L2 %02X:%02X:%02X:%02X:%02X:%02X)",
				portid,
		...
		port_mtu[portid]);
			...
			printf(	//"\nPackets received: %16"PRIu64
				...
				GREEN "\nrxs: %10"PRIu64 RESET
				GREEN " txs: %10"PRIu64 RESET
			    GREEN " ds: %10"PRIu64 RESET,
...
//                   " | Total packets dropped: %11"PRIu64
                   GREEN "\ntrxs: %9"PRIu64 RESET
                   GREEN " ttxs: %9"PRIu64 RESET
                   GREEN " tds: %9"PRIu64 RESET
...
handle_rx(struct rte_mbuf *pkt, unsigned portid)
{
	...
	buffer = tx_buffer[output_port[portid]];
	sent = rte_eth_tx_buffer(output_port[portid], 0, buffer, pkt);
	if (sent)
		port_statistics[output_port[portid]].tx += sent;
...
main(int argc, char **argv)
{
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		rte_eth_dev_set_mtu(portid, 9216);
		rte_eth_dev_get_mtu(portid, &port_mtu[portid]);

		NL
		bus_info[portid] = dev_info.device->name;
		printf("Port INFO: Driver: %s Device: %s MTU: %u", dev_info.driver_name, bus_info[portid], port_mtu[portid]);
		NL
